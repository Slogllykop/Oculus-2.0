...
export default function Toolbox() {
	...
	const applyBitrateToSender = useCallback(
		async (sender: RTCRtpSender, q: Quality) => {
			...
			for (const call of mediaCallsRef.current.values()) {
				...
				for (const sender of pc.getSenders()) {
					if (sender.track?.kind === "video")
						await applyBitrateToSender(sender, q);
				}
			}
		},
	...
	const replaceTrackOnAllCalls = useCallback(
				...
				<div className="w-64 flex flex-col gap-4">
					...
					<div className="rounded-2xl bg-zinc-950 border border-white/[0.07] p-4 space-y-3">
						...
						<p className="text-[10px] text-zinc-700 leading-relaxed">
							Enable &quot;Share system audio&quot; in the browser screen picker
							to share desktop sound.
						</p>
					</div>
